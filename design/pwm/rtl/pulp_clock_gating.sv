...
// Clock gate cell for apb_adv_timer, which instantiates pulp_clock_gating four
// times (one per timer module) but does not ship it. This maps it onto OpenTitan
// prim_clock_gating, the same cell Ibex uses. Spec: doc/src/QNSC_PWM_MAS.md, 4.
//
// The module and port names are fixed by the vendored instantiation, so they
// keep the upstream convention instead of the QNSC naming rule.
