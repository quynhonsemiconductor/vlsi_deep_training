...
| V2.3 | 2026-09-26 | Nghia VT | -- | RTL written: line indices from `qnsc_pkg` (7.1); open item 2 closed by a simulation-only X check (7.6) |
...
Inside `` `ifndef SYNTHESIS ``, a deferred assertion checks that every `i_int_*` is 0
or 1 and reports an error on X or Z. It adds no hardware. An X would otherwise pass an
OR gate silently, and Ibex checks only the whole bundle (`IbexIrqX`). A 4-state
simulator is needed to see it; Verilator is 2-state.
...
2. ~~**X on an input.**~~ Closed in V2.3: a simulation-only assertion reports it (7.6).
...
10. **Lint check**: the module contains no `i_clk_`, no `i_rst_n_`, and no
    `always_ff`.
11. An X on any input raises the 7.6 assertion (4-state simulator).
