# QNSC_SYSDBG — design decisions and record

**This is not the specification.** That is
[`QNSC_SYSDBG_MAS.md`](QNSC_SYSDBG_MAS.md): what the block is, its ports, its
registers and its behaviour — the contract verification works from.

This file is the V1.14 document as it stood before that rewrite, kept whole. It
holds the revision history, the questions that were opened and how each closed,
the reasoning behind the sourcing decision, the observations, and the host
software plan — everything that explains *why* the specification says what it
says, and everything that is true but is not a claim about this block's RTL.
