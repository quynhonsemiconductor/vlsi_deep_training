...
Save this as `main-protection.json`:
...
Notes:
- The `context` strings **must** byte-match the job `name:` values in the
  workflows. If a name changes, the required check silently stops matching and
  no longer gates merges. The five above correspond to the current names.
- `actor_id: 5` is the built-in **Admin** role, allowed to bypass only via a PR
  (not direct push). Drop the `bypass_actors` block entirely for the strictest
  setup.
